...
class P2p_Trainer():
    def __init__(self, config):
        ...
        self.dataloader = DataLoader(ImageDataset(config['train_list'], transforms_1=transforms_1, transforms_2=transforms_2, unaligned=False),
                                batch_size=config['batchSize'], shuffle=True, num_workers=config['n_cpu'])

        val_transforms = [ToTensor(),
                    Resize(size_tuple = (config['size'], config['size']))]
        
        self.val_data = DataLoader(ValDataset(config['val_list'], transforms_ =val_transforms, unaligned=False),
                                batch_size=config['batchSize'], shuffle=False, num_workers=config['n_cpu'])

        self.test_data = DataLoader(ValDataset(config['test_list'], transforms_ =val_transforms, unaligned=False),
                                batch_size=config['batchSize'], shuffle=False, num_workers=config['n_cpu'])
       # Loss plot
    ...
    def test(self,):
        ...
        out_path = self.config['image_save']
        path = 'f'
        ii = 0
        with torch.no_grad():
                MAEw = 0
